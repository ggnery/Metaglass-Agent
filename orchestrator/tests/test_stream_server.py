...
from generated import stream_pb2
from server.stream_server import StreamServer


@pytest.fixture
def service(mock_db_factory, mock_qdrant):
    return StreamServer(db_factory=mock_db_factory, qdrant=mock_qdrant)


def test_query_is_unimplemented(service, mock_servicer_context):
    request = stream_pb2.QueryRequest(session_id="s1", text_query="hello")
